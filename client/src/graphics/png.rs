...
use super::transparency::TransparencyMap;
...
/// A terrain atlas with associated transparency classification data.
pub struct TerrainAtlas {
    pub image: DedicatedImage,
    pub transparency_map: TransparencyMap,
}

/// Loader for terrain atlas that also analyzes transparency.
pub struct TerrainAtlasPng {
    pub path: PathBuf,
}

impl crate::loader::Cleanup for TerrainAtlas {
    unsafe fn cleanup(mut self, gfx: &super::Base) {
        unsafe {
            self.image.destroy(&gfx.device);
        }
    }
}

impl Loadable for TerrainAtlasPng {
    type Output = TerrainAtlas;
    ...
    fn load(self, handle: &LoadCtx) -> LoadFuture<'_, Self::Output> {
        Box::pin(async move {
            ...
            trace!(path=%full_path.display(), "loading terrain atlas with transparency analysis");

            // First, analyze transparency before reading for GPU upload
            let transparency_map = TransparencyMap::from_atlas(&full_path)?;

            // Now load the image for GPU
            ...
            match bytes_per_pixel {
                4 => mem_data.copy_from_slice(&raw_data),
                3 => {
            ...
            unsafe {
                ...
                trace!(
                    ...
                    "loaded terrain atlas with transparency data"
                );
                Ok(TerrainAtlas {
                    image,
                    transparency_map,
